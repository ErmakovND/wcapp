...
class WeatherControllerTest {
    ...
    void testWeatherBadRequest() throws XmlException {
        String location = "foo";
        int last = 1;
        when(weatherService.getLastByLocation(last, location)).thenReturn(new ArrayList<>());
        ResponseEntity<?> responseEntity = weatherController.getWeather(location, last);
        verify(weatherService, times(1)).getLastByLocation(last, location);
    ...
    void testWeatherCurrentRequest() throws XmlException {
        ...
        int last = 1;
        when(weatherService.getLastByLocation(last, location)).thenReturn(Collections.singletonList(weatherRecord));
        ResponseEntity<?> responseEntity = weatherController.getWeather(location, last);
        verify(weatherService, times(1)).getLastByLocation(last, location);
    ...
    void testWeatherHistoryRequest() throws XmlException {
        ...
        when(weatherService.getLastByLocation(last, location)).thenReturn(weatherRecords);
        ResponseEntity<?> responseEntity = weatherController.getWeather(location, last);
        verify(weatherService, times(1)).getLastByLocation(last, location);
        assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
